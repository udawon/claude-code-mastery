...
class BucketListApp {
    ...
    createBucketItemHTML(item) {
        ...
        const checkboxClass = item.completed ? 'checked' : '';
        const itemClass = item.completed ? 'completed-item' : '';

        return `
            <div class="bucket-item ${itemClass} flex items-center gap-4">
                <!-- 체크박스 -->
                <button
                    class="checkbox-btn ${checkboxClass}"
                ...
                <!-- 제목 -->
                <div class="flex-1">
                    <p class="item-title ${completedClass}">${this.escapeHtml(item.title)}</p>
                    <p class="item-date">
